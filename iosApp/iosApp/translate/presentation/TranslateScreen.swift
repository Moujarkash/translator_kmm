...
struct TranslateScreen: View {
    ...
    init(historyDataSource: HistoryDataSource, translateUseCase: Translate) {
        ...
        self.viewModel = IOSTranslateViewModel(
            historyDataSource: historyDataSource, translateUseCase: translateUseCase
        )
    }
            ...
            List {
                HStack(alignment: .center) {
                    ...
                    SwapLanguageButton(onClick: {
                        viewModel.onEvent(event: TranslateEvent.SwipeLanguages())
                    })
                    Spacer()
                    LanguageDropDown(
                ...
                TranslateTextField(
                    fromText: Binding(get: {viewModel.state.fromText}, set: {value in
                        viewModel.onEvent(event: TranslateEvent.ChangeTranslatingText(text: value))
                    }),
